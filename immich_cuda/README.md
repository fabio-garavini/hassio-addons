# Home Assistant app: Immich Cuda (all in one) 📷 by Fabio Garavini
...
- **All-in-one** — Immich Server, Machine Learning, PostgreSQL with
  VectorChord and Redis in a single app, nothing else to install
- **Automatic backup** — sync photos and videos from mobile/web clients
- **AI-powered search** — object/scene and facial recognition with Smart
  Search
...
If you want GPU-accelerated machine learning, check the **Immich
OpenVINO** (Intel) or **Immich Cuda** (NVIDIA) variants; for low-resource
devices there's **Immich NoML** (no machine learning).
