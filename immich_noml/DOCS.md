...
It's the **lightweight** variant: no machine learning service, making it
ideal for low-resource devices (Smart Search and face recognition are
unavailable). This monolithic package bundles:
...
- **Slow database** — make sure `Storage type` matches the storage the
  database runs on (`SSD` or `HDD`) so PostgreSQL is properly tuned.
...
- **[Immich](../immich/DOCS.md)** — the full variant with CPU machine
  learning (Smart Search and face recognition).
